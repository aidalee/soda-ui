import ora from 'ora'
import execa from 'execa' // 通过这个工具，我们可以在代码里执行命令行指令
...
async function isWorktreeEmpty() {
  const ret = await execa('git', ['status', '--porcelain']) // 指令查看文件变更
  return !ret.stdout
}
...
async function confirmRegistry() {
  const registry = (await execa('npm', ['config', 'get', 'registry'])).stdout // npm config get registry获取到当前镜像源
...
async function confirmRefs(remote = 'origin') {
  const { stdout } = await execa('git', ['remote', '-v']) // 获取当前代码远端地址
  const reg = new RegExp(`${remote}\t(.*) \\(push`)
  const repo = stdout.match(reg)?.[1]
  const { stdout: branch } = await execa('git', ['branch', '--show-current']) // 获取当前分支
